...
def test_create_train_X_y_output_when_series_and_exog_is_None():
    ...
    for k in results[11].keys():
        pd.testing.assert_series_equal(results[11][k], expected[11][k])
...
def test_create_train_X_y_output_when_series_10_and_exog_is_series_of_bool_str(exog_values, dtype):
    ...
    if dtype is str:
        ...
        exog_astype = {'l1': int, 'l2': int, 'exog': dtype}
        expected_cat_names = []
    ...
    expected = (
        pd.DataFrame(
            ...
            columns = ['lag_1', 'lag_2', 'lag_3', 'l1', 'l2', 'l3', 
                  ...
                  'l2': float, 'l3': float, 'exog_1': float, 'exog_2': float}
        ).astype({'l1': int, 'l2': int, 'l3': int}
        ),
        pd.Series(
    ...
    expected = (
        pd.DataFrame(
            data = np.array([[np.nan, 2., 1., np.nan, np.nan, np.nan, 1., 0., 0., 104., np.nan],
                             ...
                             [23., 22., 21., 22.0, 22.0, 86.0, 0., 0., 1., 207., 0.0]]),
            index   = pd.Index(
                          pd.DatetimeIndex(
                              ['1990-01-05', '1990-01-06', '1990-01-07', '1990-01-08',
                               '1990-01-09', '1990-01-10',
            ...
            columns = ['lag_1', 'lag_2', 'lag_3', 'roll_mean_3', 'roll_median_3', 'roll_sum_4',
                  ...
                  'l1': float, 'l2': float, 'l3': float, 'exog_1': float, 'exog_2': float}
        ).astype({'l1': int, 'l2': int, 'l3': int}
        ),
        pd.Series(

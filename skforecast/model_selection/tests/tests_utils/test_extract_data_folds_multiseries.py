...
def test_extract_data_folds_multiseries_series_DataFrame_with_NaN_exog_DatetimeIndex():
    ...
    exog_datetime = exog_wide_range.copy()
...
def test_extract_data_folds_multiseries_series_dict_exog_dict_DatetimeIndex(dropna_last_window):
    """
    Test _extract_data_folds_multiseries with series and exog as dict with
    DatetimeIndex.
    """
    series_wide_dt = series_wide_range.copy()
    series_wide_dt.index = pd.date_range(start='2020-01-01', periods=50, freq='D')

    series_dict = {
        'l1': series_wide_dt['l1'],
        'l2': series_wide_dt['l2'].iloc[10:],
        'l3': series_wide_dt['l3'].iloc[:20]
    }

    exog_wide_dt = exog_wide_range.copy()
    exog_wide_dt.index = pd.date_range(start='2020-01-01', periods=50, freq='D')

    exog_dict = {
        'l1': exog_wide_dt,
        'l2': exog_wide_dt[['exog_1', 'exog_2']].iloc[15:],
        'l3': exog_wide_dt['exog_3'].iloc[30:]
    }
...
@pytest.mark.parametrize("dropna_last_window", 
                         [True, False], 
                         ids=lambda dropna: f'dropna_last_window: {dropna}')
def test_extract_data_folds_multiseries_series_dict_exog_dict_DatetimeIndex_with_None_in_exog(dropna_last_window):
    """
    Test _extract_data_folds_multiseries with series and exog as dict with
    DatetimeIndex. Exog contains None values.
    """
    series_wide_dt = series_wide_range.copy()
    series_wide_dt.index = pd.date_range(start='2020-01-01', periods=50, freq='D')
    ...
    for i, data_fold in enumerate(data_folds):
        ...
        for key in data_fold[3].keys():
            if expected_data_folds[i][3][key] is None:
                assert data_fold[3][key] is None
            else:
                if isinstance(expected_data_folds[i][3][key], pd.DataFrame):
                    pd.testing.assert_frame_equal(data_fold[3][key], expected_data_folds[i][3][key])
                else:
                    pd.testing.assert_series_equal(data_fold[3][key], expected_data_folds[i][3][key])
        for key in data_fold[4].keys():
            if expected_data_folds[i][4][key] is None:
                assert data_fold[4][key] is None
            else:
                if isinstance(expected_data_folds[i][4][key], pd.DataFrame):
                    pd.testing.assert_frame_equal(data_fold[4][key], expected_data_folds[i][4][key])
                else:

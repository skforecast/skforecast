...
from skforecast.foundation._utils import normalize_exog_to_dict
...
def test_Chronos2Adapter_fit_trims_exog_to_context_length():
    ...
    assert len(next(iter(adapter._history_exog.values()))) == context_length
    pd.testing.assert_frame_equal(next(iter(adapter._history_exog.values())), exog.iloc[-context_length:])
...
def test_Chronos2Adapter_fit_raises_TypeError_when_series_is_invalid_type():
    ...
    err_msg = re.escape(
        ...
        f"Got {type(np.array([1, 2, 3]))}."
    )
    with pytest.raises(TypeError, match=err_msg):
        fit_adapter(adapter, series=np.array([1, 2, 3]))
...
def test_Chronos2Adapter_fit_context_length_parametrize(context_length, expected_len):
    ...
    adapter = Chronos2Adapter(model_id="autogluon/chronos-2-small", context_length=context_length)
    fit_adapter(adapter, series=y)
    assert len(next(iter(adapter._history.values()))) == expected_len
...
# ==============================================================================
def test_Chronos2Adapter_fit_multiseries_wide_dataframe_setsis_multiple_series_():
    """
    Test that fitting on a wide DataFrame sets is_multiple_series_ to True.
    """
    adapter = Chronos2Adapter(model_id="autogluon/chronos-2-small")
...
def test_Chronos2Adapter_fit_multiseries_dict_setsis_multiple_series_():
    """
    Test that fitting on a dict of Series sets is_multiple_series_ to True.
    """
    adapter = Chronos2Adapter(model_id="autogluon/chronos-2-small")
...
def test_Chronos2Adapter_fit_multiseries_shared_exog_broadcast_to_all_series():
    ...
    """
    adapter = Chronos2Adapter(model_id="autogluon/chronos-2-small")
    fit_adapter(adapter, series=_y_dict, exog=_exog_shared)
...
def test_Chronos2Adapter_fit_multiseries_per_series_exog_stored_correctly():
    ...
    """
    adapter = Chronos2Adapter(model_id="autogluon/chronos-2-small")
    fit_adapter(adapter, series=_y_dict, exog={"s1": _exog_shared.copy()})
...
def test_Chronos2Adapter_fit_multiseries_missing_exog_key_gets_none():
    ...
    """
    adapter = Chronos2Adapter(model_id="autogluon/chronos-2-small")
    fit_adapter(adapter, series=_y_dict, exog={"s1": _exog_shared})
    assert adapter._history_exog["s2"] is None
...
def test_Chronos2Adapter_fit_multiseries_raises_TypeError_for_non_series_value():
    ...
    """
    adapter = Chronos2Adapter(model_id="autogluon/chronos-2-small")
    with pytest.raises(TypeError, match="all series must be a named pandas Series"):
        fit_adapter(adapter, series={"s1": np.array([1.0, 2.0, 3.0])})

...
def test_TimesFM25Adapter_fit_returns_self():
    ...
    """
    adapter = make_adapter()
    result = fit_adapter(adapter, series=y)
    assert result is adapter
...
def test_TimesFM25Adapter_fit_does_not_share_reference_with_input():
    ...
    y_orig_values = hist.values.copy()
    y.iloc[0] = 9999.0
    np.testing.assert_array_equal(hist.values, y_orig_values)
    y.iloc[0] = 0.0  # restore


def test_TimesFM25Adapter_fit_setsis_multiple_series__false_for_series():
    """
...
def test_TimesFM25Adapter_fit_setsis_multiple_series__true_for_dataframe():
    """
...
def test_TimesFM25Adapter_fit_setsis_multiple_series__true_for_dict():
    """
...
def test_TimesFM25Adapter_predict_raises_ValueError_when_steps_exceed_max_horizon():
    ...
    with pytest.raises(ValueError, match=err_msg):
        predict_adapter(adapter, steps=15)
...
def test_TimesFM25Adapter_predict_no_warning_when_exog_provided():
    """
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        predict_adapter(adapter, steps=3, exog=future_exog)
    assert not any(issubclass(w.category, IgnoredArgumentWarning) for w in caught)


def test_TimesFM25Adapter_predict_no_warning_when_last_window_exog_provided():
    """
    Test predict does not issue IgnoredArgumentWarning at adapter level
    (handled by FoundationModel).
    """
    ...
    with warnings.catch_warnings(record=True) as caught:
        warnings.simplefilter("always")
        predict_adapter(adapter, steps=3, last_window_exog=lw_exog)
    assert not any(issubclass(w.category, IgnoredArgumentWarning) for w in caught)


# Tests TimesFM25Adapter.predict — point forecast (quantiles=None)
# ==============================================================================
def test_TimesFM25Adapter_predict_returns_dataframe_as_point_forecast():
    """
...
def test_TimesFM25Adapter_predict_series_correct_length():
    ...
    adapter = make_adapter()
    fit_adapter(adapter, series=y)
    result = predict_adapter(adapter, steps=12)
    assert len(result) == 12


def test_TimesFM25Adapter_predict_level_column_has_series_name():
    """
...
def test_TimesFM25Adapter_predict_point_forecast_values():
    ...
    """
    adapter = make_adapter()
    fit_adapter(adapter, series=data)
    result = predict_adapter(adapter, steps=12)
    expected_index = pd.date_range("1961-01-01", periods=12, freq="MS")
    pd.testing.assert_index_equal(result.index, expected_index)
    np.testing.assert_array_equal(result["pred"].to_numpy(), np.zeros(12))
...
def test_TimesFM25Adapter_predict_quantile_values_correct():
    ...
    adapter = make_adapter()
    fit_adapter(adapter, series=data)
    result = predict_adapter(adapter, steps=12, quantiles=quantiles)
    expected_index = pd.date_range("1961-01-01", periods=12, freq="MS")
    pd.testing.assert_index_equal(result.index, expected_index)
    for q in quantiles:
...
def test_TimesFM25Adapter_predict_all_supported_quantiles():
    ...
    adapter = make_adapter()
    fit_adapter(adapter, series=y)
    result = predict_adapter(adapter, steps=3, quantiles=TimesFM25Adapter.SUPPORTED_QUANTILES)
    expected_cols = ["level"] + [f"q_{q}" for q in TimesFM25Adapter.SUPPORTED_QUANTILES]

...
def test_predict_returns_DataFrame():
    """
    predict() returns a long-format pandas DataFrame.
    """
...
def test_predict_has_level_and_pred_columns():
    """
    The returned DataFrame has level and pred columns.
    """
    forecaster = make_forecaster()
    forecaster.fit(series=y)
    predictions = forecaster.predict(steps=5)
...
def test_predict_values_from_fake_pipeline():
    ...
    np.testing.assert_array_almost_equal(predictions["pred"].values, [0.5, 0.5, 0.5])

# Unit tests — ForecasterFoundation multi-series mode
# ==============================================================================
import numpy as np
import pandas as pd
import pytest
...
class TestFitMultiSeries:
    ...
    def test_fit_dataframe_setsis_multiple_series_(self):
        forecaster = make_forecaster()
        forecaster.fit(series=series_df)
        assert forecaster.is_multiple_series_ is True

    def test_fit_dict_setsis_multiple_series_(self):
        forecaster = make_forecaster()
    ...
    def test_fit_multiseries_series_names_in_no_singular(self):
        forecaster = make_forecaster()
        forecaster.fit(series=series_df)
        assert forecaster.series_names_in_ == ["s1", "s2"]
...
class TestSingleSeriesUnchanged:
    ...
    def test_fit_single_series_names_in_list(self):
        forecaster = make_forecaster()
        forecaster.fit(series=y)
        assert forecaster.series_names_in_ == ["y"]
        assert forecaster.is_multiple_series_ is False
    ...
    def test_predict_single_returns_dataframe(self):
        forecaster = make_forecaster()
        forecaster.fit(series=y)
        result = forecaster.predict(steps=5)
        assert isinstance(result, pd.DataFrame)
    ...
    def test_predict_interval_single_returns_dataframe_with_level_column(self):
        forecaster = make_forecaster()
        forecaster.fit(series=y)
        result = forecaster.predict_interval(steps=5)
        assert list(result.columns) == ["level", "pred", "lower_bound", "upper_bound"]
    ...
    def test_predict_quantiles_single_has_level_column(self):
        forecaster = make_forecaster()
        forecaster.fit(series=y)
        result = forecaster.predict_quantiles(steps=5)
        assert "level" in result.columns

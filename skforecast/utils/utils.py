...
def get_features_range(X: pd.DataFrame | pd.Series | dict) -> dict:
    """
    ...
    if isinstance(X, pd.Series):
        # TODO: check this is not changing the outside df
        X = X.to_frame()
...
def check_features_range(features_ranges: dict, X: pd.DataFrame | pd.Series, series_name: str=None) -> bool:
    """
    ...
    if isinstance(X, dict):
        for key, v in X.items():
            check_features_range(features_ranges[key], v, series_name=key)
            return

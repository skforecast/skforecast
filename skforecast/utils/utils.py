...
def _preprocess_initial_train_size(
    y: pd.Series,
    initial_train_size: Union[int, str, pd.Timestamp]
) -> int:
    """
    Return initial_train_size according to the next rules:
    - If initial_train_size is an integer, return it.
    - If initial_train_size is a string or a pandas Timestamp, and the series y has a 
    pd.DatetimeIndex, return the length of the time window between the start of the data 
    and the given date as an integer value. The given date must be included in the window.
    ...
    initial_train_size : int, str, pandas Timestamp
        Initial train size, either as an integer or a date-like string/Timestamp.

    Returns
    -------
    initial_train_size : int
        Initial train size as an integer.
    """
    if isinstance(initial_train_size, int):
        return initial_train_size
    elif isinstance(initial_train_size, (str, pd.Timestamp)):
        ...
        if isinstance(initial_train_size, str):
            try:
                initial_train_size = pd.to_datetime(initial_train_size)
            except ValueError:
                raise ValueError("The provided string cannot be converted into a pandas Timestamp.")
        if initial_train_size not in y.index:
            raise ValueError("The provided date must be in the index of y.")
        return y.index.get_loc(initial_train_size) + 1
    else:
        raise TypeError("initial_train_size must be an integer or a date in string or pandas Timestamp format.")
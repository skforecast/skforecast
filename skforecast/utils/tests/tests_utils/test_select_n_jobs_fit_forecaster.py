...
@pytest.mark.parametrize("forecaster_name, estimator, n_jobs_expected", 
    [('ForecasterDirect', LinearRegression(), 1),
     ('ForecasterDirect', HistGradientBoostingRegressor(), max(1, cpu_count() - 1)),
     ('ForecasterDirect', LGBMRegressor(), 1),
     ('ForecasterDirect', LGBMRegressor(n_jobs=1), max(1, cpu_count() - 1)),
     ('ForecasterDirectMultiVariate', LinearRegression(), 1),
     ('ForecasterDirectMultiVariate', HistGradientBoostingRegressor(), max(1, cpu_count() - 1)),
     ('ForecasterDirectMultiVariate', LGBMRegressor(), 1),
     ('ForecasterDirectMultiVariate', LGBMRegressor(n_jobs=1), max(1, cpu_count() - 1)),

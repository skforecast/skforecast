# Unit test filter_train_X_y_for_step ForecasterDirectMultiVariate
# ==============================================================================
import re
import pytest
import numpy as np
...
@pytest.mark.parametrize(
    "lags, remove_suffix, expected_X_data, expected_columns",
    [({'l1': None, 'l2': 3}, False,
      np.array([[52., 51., 50., 104.],
                [53., 52., 51., 105.],
                [54., 53., 52., 106.],
                [55., 54., 53., 107.],
                [56., 55., 54., 108.],
                [57., 56., 55., 109.]], dtype=float),
      ['l2_lag_1', 'l2_lag_2', 'l2_lag_3', 'exog_step_2']),
     ({'l1': 3, 'l2': None}, True,
      np.array([[2., 1., 0., 104.],
                [3., 2., 1., 105.],
                [4., 3., 2., 106.],
      ...
      ['l1_lag_1', 'l1_lag_2', 'l1_lag_3', 'exog'])],
    ids=['lags_l1_None-keep_suffix', 'lags_l2_None-remove_suffix']
)
def test_filter_train_X_y_for_step_output_when_lags_dict_with_None_steps_2_and_exog_for_step_2(
    lags, remove_suffix, expected_X_data, expected_columns
):
    """
    Test output of filter_train_X_y_for_step when lags is a dict with None
    for one series, steps is 2, with exog for step 2.
    """
    series = pd.DataFrame({'l1': pd.Series(np.arange(10), dtype=float), 
                           'l2': pd.Series(np.arange(50, 60), dtype=float)})
    exog = pd.Series(np.arange(100, 110), name='exog', dtype=float)
    ...
    forecaster = ForecasterDirectMultiVariate(
        estimator=LinearRegression(), level='l2', lags=lags, 
    ...
    expected_y_name = 'l2' if remove_suffix else 'l2_step_2'
    expected = (
        pd.DataFrame(
            data    = expected_X_data,
            index   = pd.RangeIndex(start=4, stop=10, step=1),
...
def test_filter_train_X_y_for_step_output_when_window_features_and_exog_steps_1(remove_suffix):
    ...
    expected_X_data = np.array(
        [[5., 4., 3., 2., 1., 3., 3., 15., 55., 54., 53., 52., 51., 53., 53., 315., 106.],
...
def test_filter_train_X_y_for_step_output_when_window_features_and_exog_steps_2(remove_suffix):
    ...
    expected_X_data = np.array(
        [[5., 4., 3., 2., 1., 3., 3., 15., 55., 54., 53., 52., 51., 53., 53., 315., 107.],
...
def test_filter_train_X_y_for_step_with_calendar_features_same_as_exog():
    ...
    X_train, y_train = forecaster.create_train_X_y(
        series=series, exog=exog_datetime
    )
    ...
    X_train_no_cal, y_train_no_cal = forecaster_no_cal.create_train_X_y(
        series=series, exog=exog_calendar
    )

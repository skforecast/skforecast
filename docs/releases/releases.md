...
+ <span class="badge text-bg-feature">Feature</span> New function <code>[reshape_series_exog_dict_to_long]</code> in module <code>[preprocessing]</code> to reshape series and the exogenous variables from a dictionary format to a long format pandas DataFrame with a MultiIndex. The first level of the index is the series name and the second level is the time index.

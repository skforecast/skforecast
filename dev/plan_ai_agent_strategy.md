...
La estrategia es **mantener el conocimiento de skforecast en dos archivos fuente** (`tools/ai/llms-base.txt` para contenido real + `llms.txt` como índice público per [llmstxt.org](https://llmstxt.org)) **y una carpeta modular `skills/`** con workflows independientes siguiendo el [Agent Skills spec](https://agentskills.io). Un script **genera automáticamente** `llms-full.txt` (concatenando `llms-base.txt` + skills) y todos los archivos de contexto para cada plataforma. Esto garantiza consistencia, elimina duplicación (~60% del contenido anterior de `llms-full.txt` era idéntico a `llms-base.txt`), y habilita *progressive disclosure* (los agentes cargan solo los skills que necesitan).
...
| **`skforecast.org/llms.txt`** | LLMs con web search (ChatGPT, Perplexity, etc.) | Usuarios que piden usar skforecast a un LLM | Cuando busca info de skforecast (índice per llmstxt.org spec) |
| **`skforecast.org/llms-full.txt`** | Cualquier LLM cuando el usuario pega la URL | Todos | Bajo demanda (contenido completo expandido) |
...
### La solución: `llms.txt` (índice) + `llms-base.txt` (contenido) + `skills/` + 1 script

Se mantienen manualmente:

| Fuente | Contenido | Formato | Para quién |
|--------|-----------|---------|------------|
| `llms.txt` | Índice público per [llmstxt.org](https://llmstxt.org) spec: links a docs, API, guías | Markdown plano (~120 líneas) | LLMs con web search, crawlers, cualquier usuario |
| `tools/ai/llms-base.txt` | Core reference: estructura del proyecto, imports, API overview, ejemplos básicos | Markdown plano (~730 líneas) | IDEs (auto-inyectado), base de `llms-full.txt` |
...
| `tools/ai/ai_context_header.md` | Header de desarrollo: tests, code style, contribución | Markdown plano (~30 líneas) | Solo devs del repo |

**`llms-full.txt` ya NO se mantiene manualmente** — se genera automáticamente concatenando `llms-base.txt` + todos los `SKILL.md` (sin frontmatter). Esto elimina la duplicación que existía (~60% del contenido era idéntico).

**Decisión de diseño — por qué `skills/` en vez de un monolito**:

1. **Eliminación de duplicación**: Antes, `llms-full.txt` repetía ~430 líneas idénticas de `llms-base.txt`. Con skills, cada pieza de contenido existe en un solo lugar.
2. **Progressive disclosure**: Un agente AI carga `llms-base.txt` (~730 líneas) por defecto; solo activa skills específicos cuando necesita un workflow concreto. Esto reduce el consumo de tokens a lo estrictamente necesario.
...
4. **Distribución universal**: Los skills se distribuyen a través de múltiples canales: clonado del repo (disponibles automáticamente para agentes locales), sistema de plugins de Claude Code (`/plugin install`), y concatenados en `llms-full.txt` para cualquier LLM.
5. **Estándar abierto con adopción creciente**: El [Agent Skills spec](https://agentskills.io/specification) es un estándar abierto iniciado por Anthropic y adoptado por GitHub, Databricks, Goose, OpenHands, Mistral, Spring AI, TRAE, entre otros. Validación oficial via [`skills-ref validate`](https://github.com/agentskills/agentskills/tree/main/skills-ref). No atamos el contenido a un proveedor.

**Decisión de diseño — IDE context**: Los archivos de IDE usan `llms-base.txt` (corto), **no** `llms-full.txt`. `llms-base.txt` (~730 líneas) ya contiene la referencia de API, imports y ejemplos — suficiente para un dev con IDE que tiene el código delante. Inyectar `llms-full.txt` (>2000 líneas) en cada prompt causaría gasto innecesario de tokens, truncamiento, y dilución del contenido importante.

**Decisión de diseño — `llms.txt` como índice**: Siguiendo la especificación [llmstxt.org](https://llmstxt.org), `llms.txt` es un **índice breve** (~120 líneas) con links a documentación, no contenido real. Los LLMs con web search lo usan como punto de entrada. El contenido real está en `tools/ai/llms-base.txt` y se expande a `llms-full.txt`.

**Versionado**: Incluir una línea prominente al inicio de `llms-base.txt`: `"This document is for skforecast v0.21.0+. If you are using an older version, check the documentation at skforecast.org."`. Mantener versiones archivadas probablemente no compensa el esfuerzo.

Un script `tools/ai/generate_ai_context_files.py` genera automáticamente `llms-full.txt` y todos los archivos derivados. Ver [sección 1: Arquitectura de archivos](#arquitectura-de-archivos) para el mapeo completo.

### Flujo de actualización (cada release)

```
1. Editar tools/ai/llms-base.txt y/o skills/ con los cambios de API
   Editar llms.txt solo si hay nuevas páginas de docs
2. Ejecutar: python tools/ai/generate_ai_context_files.py
   → Genera llms-full.txt (llms-base.txt + skills sin frontmatter)
   → Genera archivos de IDE (header + llms-base.txt)
   → Copia llms.txt (índice) y llms-full.txt a docs/
...
### Script `tools/ai/generate_ai_context_files.py` — diseño

El script debe implementar esta lógica:

```python
# 1. Ensamblar llms-full.txt = llms-base.txt + todos los SKILL.md + references/ (sin frontmatter YAML)
...
llms_full = llms_base_txt + "\n\n" + skills_content

# 2. Archivos de IDE: header + llms-base.txt (corto)
dev_content = AUTOGEN_NOTICE_IDE + dev_header + "\n\n" + llms_base_txt

# 3. Copiar llms.txt (índice) y llms-full.txt a docs/ para web
```

Donde `AUTOGEN_NOTICE` tiene dos variantes según el archivo destino:

Para archivos IDE (header + llms-base.txt):
```
...
Para `llms-full.txt` (llms-base.txt + skills):
```
<!-- AUTO-GENERATED FILE. DO NOT EDIT MANUALLY. -->
...
| Función | Descripción |
|---------|-------------|
| Ensamblaje `llms-full.txt` | Concatena `llms-base.txt` + todos los `skills/*/SKILL.md` (sin frontmatter YAML) + `references/*.md` si existen, en orden explícito definido en `SKILL_ORDER` |
| Generación IDE | Lee header + `llms-base.txt`, genera los 7 archivos de IDE (ver arquitectura) |
...
| Validación de fuentes | Verificar que `llms-base.txt` contiene los imports de los `__init__.py` públicos, y que la versión coincide con `skforecast/__init__.py` |
...
| # | Tarea | Ejecutor | Estado | Notas |
|---|-------|----------|--------|-------|
| 1.2 | Crear `tools/ai/ai_context_header.md` con info de desarrollo | 🤖 | ⬜ | Testing, code style, contribución — solo para devs del repo |
...
| 1.5 | Crear script `tools/ai/generate_ai_context_files.py` | 🤖 | ⬜ | Ensambla llms-full.txt (llms-base.txt + skills + references), genera archivos IDE, valida skills + imports, modo `--check`. Se escribe ahora pero se **ejecuta en Fase D** (necesita skills) |
...
| 1.8 | Crear skill `troubleshooting-common-errors` | 🤖 | ⬜ | Errores frecuentes de LLMs + soluciones |
...
| 2.2 | Añadir step de CI para validar archivos generados | 🤖 | ⬜ | `python tools/ai/generate_ai_context_files.py --check` en CI existente |
...
```
skforecast/
├── llms.txt                              ← FUENTE 1: Índice público per llmstxt.org (~120 líneas, mantener manualmente)
...
├── tools/
│   └── ai/
│       ├── llms-base.txt                 ← FUENTE 3: Core reference (~730 líneas, mantener manualmente)
│       ├── ai_context_header.md          ← FUENTE 4: Header de desarrollo (mantener manualmente)
│       └── generate_ai_context_files.py  ← Script que genera todo
│
│  ── ARCHIVOS GENERADOS (NO editar manualmente) ──
├── llms-full.txt                         ← Generado: llms-base.txt + todos los SKILL.md (sin frontmatter)
├── .github/copilot-instructions.md       ← Generado: header + llms-base.txt (corto)
├── AGENTS.md                             ← Generado: header + llms-base.txt (corto)
├── .claude/CLAUDE.md                     ← Generado: header + llms-base.txt (corto)
├── .windsurfrules                        ← Generado: header + llms-base.txt (corto)
├── .cursor/rules/skforecast.mdc          ← Generado: frontmatter + header + llms-base.txt (corto)
├── .github/instructions/
...
└── docs/
    ├── llms.txt                          ← Generado: copia del índice para web
...
#### Contenido de `tools/ai/ai_context_header.md`
...
description: >
  Forecasts a single time series using ForecasterRecursive with
  scikit-learn compatible estimators. Covers training, prediction,
  backtesting and evaluation. Use when the user wants to predict
  future values of one time series.
---
```
...
|-------|-----------|---------------|
| `name` | ✅ | Max 64 chars, lowercase, solo `a-z`, `0-9`, `-`. Debe coincidir con el nombre del directorio padre. No puede empezar ni terminar con `-`. No puede contener hyphens consecutivos (`--`). No puede contener XML tags ni palabras reservadas (`anthropic`, `claude`) |
| `description` | ✅ | Max 1024 chars, tercera persona orientada a acción ("Forecasts...", "Generates..."), NO "Teaches the agent..." ni "You can...". Incluir qué hace + cuándo usarlo ("Use when..."). Sin mencionar versiones. No puede contener XML tags |
| `license` | ❌ | String (ej. "MIT") |
| `compatibility` | ❌ | Max 500 chars. Requisitos de entorno (producto, paquetes de sistema, acceso a red). La mayoría de skills no lo necesitan |
| `metadata` | ❌ | Mapa key-value para propiedades adicionales (ej. `author: skforecast`, `version: "1.0"`). Usar nombres de clave razonablemente únicos para evitar conflictos |
| `allowed-tools` | ❌ | Lista de tools pre-aprobados separados por espacios (experimental, soporte varía entre agentes) |
...
2. **Tercera persona orientada a acción en `description`**: Describir qué hace el skill ("Forecasts...", "Generates...", "Detects...") + cuándo usarlo ("Use when..."). NO usar "Teaches the agent...", "Learn how to...", "You can..." ni "I can help you...". La description es crítica para la selección automática entre 100+ skills: debe permitir al agente saber cuándo activar este skill.
...
|-------|--------------------------|---------------------|-------------|
| `forecasting-single-series` | Forecasts a single time series using ForecasterRecursive with scikit-learn compatible estimators. Covers training, prediction, backtesting and evaluation. Use when the user wants to predict future values of one time series. | Workflow completo: imports, datos, fit, predict, backtesting, plot | ~150 |
| `forecasting-multiple-series` | Forecasts multiple related time series simultaneously using ForecasterRecursiveMultiSeries with a single global model. Use when the user has several time series and wants to predict them all together. | Workflow con series dict/DataFrame, encoding, predict por levels | ~150 |
| `hyperparameter-optimization` | Finds optimal hyperparameters and lag configurations using grid, random, and Bayesian search with cross-validation. Use when the user wants to tune a forecaster or compare configurations. | grid_search, random_search, bayesian_search con TimeSeriesFold | ~200 |
| `statistical-models` | Fits and forecasts with ARIMA, ETS, SARIMAX and ARAR statistical models via ForecasterStats. Includes automatic order selection (Auto-ARIMA). Use when the user asks for ARIMA, exponential smoothing, or traditional statistical forecasting. | Arima, Auto-ARIMA, Ets, Sarimax con fit/predict | ~150 |
| `prediction-intervals` | Generates prediction intervals using bootstrapping and conformal methods. Use when the user needs uncertainty estimates, confidence bands, or probabilistic forecasts. | predict_interval, métodos, backtesting con intervals | ~150 |
| `feature-engineering` | Creates lag-based and rolling window features using RollingFeatures, TimeSeriesDifferentiator and DateTimeFeatureTransformer. Use when the user wants to add engineered features to a forecaster. | RollingFeatures, TimeSeriesDifferentiator, DateTimeFeatureTransformer | ~120 |
| `feature-selection` | Selects optimal lags, window features and exogenous variables using sklearn selectors like RFECV. Use when the user wants to reduce features or identify the most important predictors. | select_features, select_features_multiseries con RFECV | ~100 |
| `drift-detection` | Detects data drift in production using range-based and population-based statistical tests. Use when the user wants to monitor model inputs for distribution changes after deployment. | RangeDriftDetector, PopulationDriftDetector | ~120 |
| `deep-learning-forecasting` | Forecasts time series using RNN/LSTM neural networks with ForecasterRnn and Keras. Use when the user wants deep learning forecasting or mentions neural networks, RNN, or LSTM. | create_and_compile_model, ForecasterRnn | ~120 |
| `choosing-a-forecaster` | Recommends the appropriate skforecast forecaster based on data characteristics and requirements. Use when the user is unsure which forecaster to use or asks for guidance on approach selection. | Tabla de decisión, diagrama de flujo, recomendaciones | ~100 |
| `troubleshooting-common-errors` | Diagnoses and fixes common errors when using skforecast, including deprecated imports, missing frequencies, and exogenous variable mismatches. Use when the user encounters an error or when generated code fails. | Tabla error-causa-solución, deprecated imports | ~80 |
| `complete-api-reference` | Provides complete constructor and method signatures for all skforecast forecasters and functions. Use when the user needs exact parameter names, types, or defaults for any skforecast class or function. | Índice de firmas + referencia a `references/method-signatures.md` | ~100 + refs |
...
Los skills se distribuyen a través de múltiples canales:

| Canal | Mecanismo | Acción del usuario |
|-------|-----------|-------------------|
| **Claude Code (plugins)** | `/plugin marketplace add skforecast/skforecast` o `/plugin install` directo | El dev ejecuta el comando en Claude Code |
| **Claude.ai** | Subir `SKILL.md` como skill en la interfaz web | El usuario sube el archivo manualmente |
| **Claude API** | Usar la [Skills API](https://docs.claude.com/en/api/skills-guide) para registrar skills programáticamente | El dev lo integra en su aplicación |
| **Clonado del repo** | `skills/` ya está en el repo | Disponible automáticamente para agentes locales (Claude Code, Cursor, etc.) |
| **`llms-full.txt`** | El script concatena todos los skills en un solo archivo | El usuario pega la URL o lo sube como knowledge |
| **Validación** | `skills-ref validate ./skills/my-skill` verifica frontmatter y naming | Usar antes de cada release |
...
**C) Skill `troubleshooting-common-errors`** — Errores comunes que los LLMs suelen cometer:
...
VS Code + Copilot                  →  "Mira ai_changes_pending.md y actualiza:
                                       - tools/ai/llms-base.txt si cambian imports, API o estructura
                                       - skills/ si cambian workflows o ejemplos
                                       - llms.txt si hay nuevas páginas de docs"
        ↓
python tools/ai/generate_ai_context_files.py  →  Limpiar ai_changes_pending.md → commit
```
...
| Que el workflow edite `llms-base.txt` / `skills/` con un LLM | Riesgo de que el LLM malinterprete un cambio. Las fuentes de verdad no deben editarse automáticamente |
...
- **Claude Code**: `skills/` auto-detected, or install via `/plugin install`. Also reads `AGENTS.md` and `.claude/CLAUDE.md`
...
| 3 | Actualizar `tools/ai/llms-base.txt` (imports, API, estructura) y/o `skills/` (workflows, ejemplos) según los cambios detectados. Actualizar `llms.txt` si hay nuevas páginas de docs |
| 4 | Limpiar `tools/ai_changes_pending.md` (dejar solo el header) |
| 5 | Ejecutar `python tools/ai/generate_ai_context_files.py` |
...
- Adopción de skills: clones del repo con `skills/`, instalaciones via Claude Code plugins, descargas de skills individuales

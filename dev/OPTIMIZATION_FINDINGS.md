...
| 5 | single-series | `_create_train_X_y` | **REJECTED** — see revision below | none (claim not reproducible; regresses lags-only path + correctness bug) |
| 6 | single-series | `_create_predict_inputs` | yes | 5–15% off each `predict()` call |

Findings #1, #2, #4 are the highest-leverage; #6 is smaller but very cheap to implement. #3 is included for completeness with corrected scope. **#5 was rejected on a third review** (see the revision note in its section) — the speedup is not reproducible and both proposed changes carry downside.
...
> ⛔ **REJECTED in third review (benchmark: [benchmark_finding_5.py](dev/benchmark_finding_5.py)).**
> Both proposed changes were tested against the real `_create_train_X_y` code path
> (capturing the exact column blocks it concatenates, with their real strides/dtypes).
> Neither is worth making:
>
> - **Change 1 (preallocate instead of `np.concatenate`) does not deliver the claimed 15–25%.**
>   The premise — "one fewer full copy of `X_train`" — is **wrong**. The `np.concatenate`
>   at line 916 is not an *extra* copy on top of the block allocations; it *is* the single
>   combining copy. Preallocation relocates the same byte-volume into per-block slice writes,
>   so total data movement is identical. Best-of-7 timing of the assembly step in isolation is
>   within noise (−9.5% … +23.9%, no systematic direction), and because the assembly is only a
>   small fraction of the whole function, the **projected best-case full-function speedup is
>   ≤ +1.1%** across all scenarios (lags+WF+exog @ 2k/10k/100k rows, exog with 5 and 20 cols).
> - **Change 1 regresses the common lags-only path.** `_create_lags` returns a reversed *view*
>   (`strides=(8, -8)`, zero copies) and the current single-block branch returns it directly.
>   Preallocation forces a full copy there (~19,000× slower on that branch in the benchmark).
> - **Change 2 (`pd.isna(X_train)` → `np.isnan(X_train)`) is a correctness regression.**
>   When `exog` has a non-float column (category / datetime / object), `exog.to_numpy()` makes
>   `X_train` object-dtype at line 916; `np.isnan` then raises `TypeError`, while `pd.isna`
>   handles it. (Line 919's `np.isnan(y_train)` is safe only because `y_train` is always float.)
>
> The original "what the code does / why it's slow / proposed change" text below is retained for
> the record, but the conclusion is: **do not implement #5.** Run `conda run -n skforecast_22_py13
> python dev/benchmark_finding_5.py` to reproduce.
...
- ~~~15–25% faster `_create_train_X_y` per fit.~~ **Not reproducible — see the rejection note at the top of this section.** Measured projected full-function gain is ≤ +1.1%, and the change regresses the lags-only path.
- ~~Helps every backtest fold that retrains and every Optuna trial.~~
- ~~Memory: removes one transient `X_train`-sized allocation.~~ Incorrect — `np.concatenate` and preallocation move the same byte-volume; neither holds fewer arrays live during the copy.
...
- **The `pd.isna` → `np.isnan` half of this finding is unsafe** (object-dtype `X_train` from non-float exog raises `TypeError`). Do not apply it.
...
2. **#6** — trivial, mostly just removing redundant work.
3. **#4** — biggest absolute win for Bayesian-search users; highest refactor cost.
4. **#2** — high impact for multiseries with many levels; moderate refactor (changes the internal contract between `_create_predict_inputs` and `_recursive_predict`).
5. **#3** — only if #2 is also implemented; otherwise the standalone gain is too small to justify.

**#5 — rejected, do not implement** (see the revision note in its section; verified by `dev/benchmark_finding_5.py`).

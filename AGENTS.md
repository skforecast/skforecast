...
- In `backtesting_foundation`, a series is predicted in a fold only if it has at least one observed value in that fold's test window; its context is the series up to the end of the train span, NaN included, so every prediction is dated inside the fold.

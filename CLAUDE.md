...
- **Task workflows** — [skills/](skills/) contains one folder per workflow (baseline-forecasting, choosing-a-forecaster, complete-api-reference, deep-learning-forecasting, drift-detection, feature-engineering, feature-selection, forecasting-multiple-series, forecasting-single-series, foundation-forecasting, hyperparameter-optimization, metric-selection, prediction-intervals, statistical-models, troubleshooting-common-errors). Before implementing or explaining a workflow that matches one of these, read the matching `SKILL.md` (and its `references/` if present).
